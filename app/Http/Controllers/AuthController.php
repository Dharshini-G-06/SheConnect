...
class AuthController extends Controller
{
    ...
    public function adminDashboard()
    {
        ...
        $pendingSOS = SosRequest::where(
            'status',
            'Pending'
        )->count();

        $resolvedSOS = SosRequest::where(
            'status',
            'Resolved'
        )->count();
    ...
    public function adminComplaints()
    {
        ...
        $pending = Complaint::where(
            'status',
            'Pending'
        )->count();

        $progress = Complaint::where(
            'status',
            'In Progress'
        )->count();

        $resolved = Complaint::where(
            'status',
            'Resolved'
        )->count();
    ...
    public function updateHostelComplaint(
        Request $request,
        $id
    ) {
        $request->validate([
    ...
    public function updateHostelLeave(
        Request $request,
        $id
    ) {
        $request->validate([
    ...
    {
        $complaints =
            HostelComplaint::with('student')
                ->latest()
                ->get();

        $leaves =
            HostelLeave::with('student')
                ->latest()
                ->get();

        $passes =
            VisitorPass::with('student')
                ->latest()
                ->get();
    ...
    public function saveVisitor(
        ...
        return back()->with(
            'success',
            'Visitor Pass Request Submitted Successfully'
        );
    }
    ...
    {
        $scholarships =
            Scholarship::latest()->get();

        $applications =
            ScholarshipApplication::where(
                'student_id',
                session('student_id')
            )->get();
    ...
    public function updateScholarshipStatus(
        ...
        $application =
            ScholarshipApplication::findOrFail($id);
